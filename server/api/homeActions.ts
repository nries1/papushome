...
export async function controlLight(
  ...
  if (!HA_TOKEN) {
    return {
      success: false,
      summary:
        'Home Assistant is not configured (missing HOMEASSISTANT_TOKEN).',
    }
  }
  ...
  if (command.action === 'turn_on') {
    if (command.brightness_pct !== undefined) {
      payload.brightness_pct = Math.max(
        1,
        Math.min(100, command.brightness_pct)
      )
    }
  ...
  if (command.action === 'turn_on') {
    if (command.brightness_pct !== undefined)
      verb += ` at ${command.brightness_pct}%`
